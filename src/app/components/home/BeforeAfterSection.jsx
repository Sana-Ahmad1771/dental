...
export default function BeforeAfterSection() {
  ...
  return (
    <section className="bg-white py-24 relative overflow-hidden">
      <div className="relative max-w-[1600px] mx-auto px-6 lg:px-16 xl:px-24 flex flex-col lg:flex-row-reverse items-center gap-12 lg:gap-20">
        {/* LEFT CONTENT */}
        ...
        <div
          ref={containerRef}
          className="lg:w-1/2 relative w-full max-w-[900px] h-[460px] rounded-2xl overflow-hidden shadow-[0_8px_30px_rgba(0,0,0,0.15)] select-none border border-primary/10"
        ...
        >
          ...
          {/* HANDLE BUTTON */}
          <div
            className="absolute top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-primary text-light border border-light flex items-center justify-center z-30 shadow-lg cursor-grab active:cursor-grabbing transition-transform duration-200"
            style={{ left: `calc(${dividerPosition}% - 20px)` }}
            onMouseDown={handleMouseDown}
            onTouchStart={() => setIsDragging(true)}
          >
            <div className="text-light font-bold">|||</div>
          </div>

          {/* LABELS */}
          <div className="absolute top-4 left-4 bg-primary/80 text-light text-sm font-semibold px-3 py-1 rounded shadow-sm">
            Before
          </div>
          <div className="absolute top-4 right-4 bg-secondary/80 text-light text-sm font-semibold px-3 py-1 rounded shadow-sm">
            After
          </div>
        </div>
      </div>
